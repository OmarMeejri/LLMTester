...
| Checkout | Clones the repo over SSH at the requested `GIT_REF` |
...
- `regression` tests are P1 (target ≥95% pass rate) — currently enforced the same as P0 given the suite's small size (4 tests). Revisit with a pass-rate threshold script once the regression suite is large enough for partial tolerance to matter.

## Running the pipeline locally

```bash
./scripts/ci-local.sh          # mirrors the full CI pipeline: install, lint, test, coverage
./scripts/test-changed.sh      # runs only test files changed vs origin/develop
```
...
| `GIT_REF` | `develop` | Branch/ref to build |
...
- **Checkout fails with credential errors**: confirm the `gitlab-ssh-credentials` Jenkins credential exists and has access to `git@gitlab.dom.tti:geniatesting/LLMTester.git`.
